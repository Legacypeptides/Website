...
import { sendOrderToPodio } from '../services/podio';
...
export const Checkout: React.FC<CheckoutProps> = ({ isOpen, onClose, onBack }) => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      await fetch('https://hook.us2.make.com/l914yer7bshfm0qruhh33dmskzkmxc6m', {
        method: 'POST',
        headers: {
      ...
      // Send order to Podio
      try {
        await sendOrderToPodio(orderData);
        console.log('Order sent to Podio successfully');
      } catch (podioError) {
        console.error('Failed to send order to Podio (non-critical):', podioError);
        // Don't fail the order if Podio fails - it's a notification system
      }
